...
class ProfitMarketer_Updater {
	...
	/**
	 * Instantiate ProfitMarketer_Plugin_Updater and ProfitMarketer_Theme_Updater
	 * for proper user capabilities.
	 */
	public static function init() {
		if ( current_user_can( 'update_plugins' ) ) {
			new ProfitMarketer_Plugin_Updater;
		}
		if ( current_user_can( 'update_themes' ) ) {
			new ProfitMarketer_Theme_Updater;
		}
	}
	...
	*/
	protected function get_local_plugin_meta( $headers ) {
		...
		foreach ( (array) self::$extra_headers as $key => $value ) {
			...
			switch( $value ) {
				case 'Profit Marketer Plugin URI':
					if ( empty( $headers['Profit Marketer Plugin URI'] ) ) {
						break;
					}
					$git_repo['type']         = 'github_plugin';

					$owner_repo               = parse_url( $headers['Profit Marketer Plugin URI'], PHP_URL_PATH );
					...
					break;
				case 'Profit Marketer Branch':
					if ( empty( $headers['Profit Marketer Branch'] ) ) {
						break;
					}
